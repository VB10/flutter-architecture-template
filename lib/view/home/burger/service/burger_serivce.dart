...
abstract class IBurgerService {
  ...
  Future<List<BurgerModel>> fetchBurgersSorted({
    ...
    BurgerSortValuesType types = BurgerSortValuesType.ASC,
  });
}
...
class BurgerService extends IBurgerService with NetworkHelper {
  ...
  Future<List<BurgerModel>> fetchBurgersLimited({double? max = 50, double? min = 0}) async {
    final response = await manager.send<BurgerModel, List<BurgerModel>>(
      ...
      queryParameters: Map.fromEntries(
        [BurgerQueryParamaters.MIN.rawValue(min?.toInt()), BurgerQueryParamaters.MAX.rawValue(max?.toInt())],
      ),
    );
  ...
  Future<List<BurgerModel>> fetchBurgersSorted({
    ...
    final response = await manager.send<BurgerModel, List<BurgerModel>>(
      ...
      method: RequestType.GET,
      queryParameters: Map.fromEntries([
        BurgerQueryParamaters.SORT.rawValue(sort.rawValue),
        BurgerQueryParamaters.SORT_TYPE.rawValue(types.rawValue)
      ]),
    );
  ...
  Future<List<BurgerModel>> fetchFavoriteBurgers() async {
    final response = await manager.send<BurgerModel, List<BurgerModel>>(
      ...
      parseModel: BurgerModel(),
      method: RequestType.GET,
      queryParameters: Map.fromEntries([BurgerQueryParamaters.FAVORITE.rawValue(true)]),
    );
    showLog(response);
    return response.data ?? [];
  }
